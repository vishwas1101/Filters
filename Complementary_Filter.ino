...
void setup(){
  ...
  for(int i = 0; i<500; i++){
    ...
    accError[2] = accError[2] + ((atan((accRaw[2])/sqrt(pow((accRaw[0]),2) + pow((accRaw[1]),2)))*RAD2DEG));
   }
  for(int i = 0; i<3; i++){
...
void loop() {
  ...
  //Getting accelerometer values and angles from them 
  Wire.beginTransmission(0x68);
  Wire.write(0x3B);
  Wire.endTransmission(false);
  Wire.requestFrom(0x68, 6, true);
  ...
  accAngle[2] = (atan((accRaw[2])/sqrt(pow((accRaw[0]),2) + pow((accRaw[2]),2)))*RAD2DEG) - accError[2];
